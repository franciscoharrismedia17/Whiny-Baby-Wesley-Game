...
const MAGA_NO_OVERLAY_CONFIG = {
  offset: { x: 0, y: 0 },
  scale: 1.8
};
const OIL_NO_OVERLAY_CONFIG = {
  offset: { x: 0, y: 0 },
  scale: 1.8
};
const MAGA_OVERLAY_POSITION = { x: WIDTH * 0.25, y: HEIGHT * 0.23 };
const OIL_OVERLAY_POSITION  = { x: WIDTH * 0.25, y: HEIGHT * 0.23 };
...
function drawDropOverlays(){
  if (!dropOverlay || !dropOverlay.img) return;
  push();
  imageMode(CENTER);
  tint(255, 255);
  image(dropOverlay.img, dropOverlay.x, dropOverlay.y, dropOverlay.w, dropOverlay.h);
  pop();
}
...
function spawnDropOverlay(img, x, y, w, h, scale = 1){
  if (!img) return;
  const baseW = (typeof w === 'number' && !Number.isNaN(w)) ? w : (img.width || 0);
  const baseH = (typeof h === 'number' && !Number.isNaN(h)) ? h : (img.height || 0);
  ...
  if (droppedInside){
    ...
    if (obj.key === 'hillary'){
      ...
      if (obj.key === 'maga'){
        spawnDropOverlay(
          imgMagaNo,
          MAGA_OVERLAY_POSITION.x + MAGA_NO_OVERLAY_CONFIG.offset.x,
          MAGA_OVERLAY_POSITION.y + MAGA_NO_OVERLAY_CONFIG.offset.y,
          dropW,
          dropH,
          MAGA_NO_OVERLAY_CONFIG.scale
        );
      } else if (obj.key === 'oil'){
        spawnDropOverlay(
          imgOilNo,
          OIL_OVERLAY_POSITION.x + OIL_NO_OVERLAY_CONFIG.offset.x,
          OIL_OVERLAY_POSITION.y + OIL_NO_OVERLAY_CONFIG.offset.y,
          dropW,
          dropH,
          OIL_NO_OVERLAY_CONFIG.scale
        );
      }
