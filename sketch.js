...
const NEGATIVE_OVERLAY_SCALE = 1.8;
const MAGA_OVERLAY_OFFSET = { x: 0, y: 0 };
const OIL_OVERLAY_OFFSET = { x: 0, y: 0 };
...
function spawnDropOverlay(img, x, y, w, h){
  ...
  const scaledW = baseW * NEGATIVE_OVERLAY_SCALE;
  const scaledH = baseH * NEGATIVE_OVERLAY_SCALE;
  dropOverlay = {
    img,
    x,
    y,
  ...
  if (droppedInside){
    ...
    if (obj.key === 'hillary'){
      ...
      if (obj.key === 'maga'){
        spawnDropOverlay(
          imgMagaNo,
          dropX + MAGA_OVERLAY_OFFSET.x,
          dropY + MAGA_OVERLAY_OFFSET.y,
          dropW,
          dropH
        );
      } else if (obj.key === 'oil'){
        spawnDropOverlay(
          imgOilNo,
          dropX + OIL_OVERLAY_OFFSET.x,
          dropY + OIL_OVERLAY_OFFSET.y,
          dropW,
          dropH
        );
      }
